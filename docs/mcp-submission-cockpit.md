...
Last updated: `2026-08-16T20:46:22Z`
...
| MCP Market | https://mcpmarket.com/submit | Prepared / free queue selected | Repository and free option are complete | Submitter must enter their own email and confirm final submit | Browser form | Do not switch to the paid expedited option. |
| MCP Marketplace | https://mcp-marketplace.io/server/bio-atlarium-habitat-database | In review / security scan | Monitor the updated canonical listing | External scan/review; a duplicate manual submission is superseded by the canonical update | `config/distribution-registry.json` | Canonical listing updated to `2.0.1` with full metadata, health check, changelog and re-scan request. |
