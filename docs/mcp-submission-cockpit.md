...
| Glama | https://glama.ai/mcp/connectors/bio.atlarium/habitat-database | Indexed as connector; ownership claim file live | Verify claim file and prepare claim copy | Glama web claim if UI asks for confirmation | `https://mcp.atlarium.bio/.well-known/glama.json` | Complete claim in browser, then mark accepted only if owner state is visible. |
| Smithery | https://smithery.ai/new | Pending maintainer submission | Prepare endpoint, repo and copy | Atlarium/TechGardeners OAuth/login | `tmp/directory-submissions/secondary-directory-payloads.md` after `pnpm directories:submit -- --payload` | Submit remote endpoint, server card and repo from a logged-in browser. |
