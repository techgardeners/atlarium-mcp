...
Last updated: `2026-08-18T19:21:29Z`
...
| Smithery | https://smithery.ai/servers/ilgrafico79/atlarium-habitat-database | Published / visible; metadata stale at 3 legacy resources | Keep release metadata healthy and monitor the existing listing | The signed-in account lacks the `ilgrafico79` owner namespace; support must transfer access or refresh the record | `config/distribution-registry.json` | Email sent to `contact@smithery.ai` requesting ownership access and a `2.0.3` / v4 refresh without creating a duplicate. Follow up at D7/D14/D30. |
| MCP.so | https://mcp.so/servers/atlarium-mcp | Listed / visible; ownership linkage verified | Monitor listing metadata and slug | Account is linked; directory support is required for the edit-form persistence defect | `config/distribution-registry.json` | Complete `2.0.3` metadata sent to `support@mcp.so` after the edit form failed to persist. Follow up at D7/D14/D30 and do not create a duplicate. |
