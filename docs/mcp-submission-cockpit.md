...
| Smithery | https://smithery.ai/servers/ilgrafico79/atlarium-habitat-database | Published / visible; score `88/100` | Keep release metadata healthy; monitor score after metadata changes | Smithery API key or manual UI file picker only for custom icon upload | `docs/assets/chatgpt-app-icon.png` for icon; `tmp/directory-submissions/secondary-directory-payloads.md` after `pnpm directories:submit -- --payload` | Optional: upload custom icon to recover the remaining icon-score points; no badge until badge policy is approved. |
